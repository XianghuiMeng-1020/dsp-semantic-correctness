...
We evaluate generated DSP implementations by specification-set
membership: a candidate is correct if it satisfies a frozen
passband, stopband, and stability contract, independently of whether
its coefficients match a canonical design.
...
with \texttt{firwin} or \texttt{butter} ($4/4$ tasks, $2/3$ model
families, $6/12$ cells).
...
Coefficient and response-to-reference oracles false-reject
tone-validated implementations that the specification accepts.
Correctness is membership in the feasible set, not agreement with one
realization.
...
The last is the correctness object.
...
These coincide if and only if $\mathcal{V}_t$ is a singleton up to
$\tau_R$.
...
Three oracles are scored against a signal-domain battery $T$: real
cosines of length $8192$ are passed through \texttt{lfilter}, and
steady-state Hilbert envelopes are compared with the same mask.
...
while every control pair exceeds $\tau_R$.
...
(Table~\ref{tab:n}).
The event occupies $4/4$ tasks, $2/3$ model families (Llama contributes
none), and $6/12$ cells.
...
$13.5$--$30.5$ samples.
The elliptic pair is not gold-equivalent on the mask:
...
\noindent\textbf{RQ4.}
Table~\ref{tab:oracle} compares oracles with $T$.
Oracle~C coincides with $T$ on all $14$ eligible implementations
($9/9$ specification-valid filters are fully in-mask; $5/5$
specification failures miss at least one constrained tone).
Oracle~A agrees on $5/14$ and false-rejects every tone-valid
implementation ($\mathrm{FRR}=1$, $\mathrm{FAR}=0$).
Oracle~B agrees on $8/14$ and still false-rejects $6/9$ tone-valid
filters ($\mathrm{FRR}=0.67$), namely the different-order FIR designs
and the elliptic pair.
The three tone-valid filters that pass Oracle~B---two same-order
band-stops and the Butterworth $(4,4)$---still fail Oracle~A.
...
Tasks / models / cells & $4/4$ / $2/3$ / $6/12$ \\
...
\begin{table}[t]
\caption{RQ4: oracles versus the constrained-tone battery $T$
($n=14$ eligible implementations).
False rejection is oracle fail and $T$ pass; false acceptance is
oracle pass and $T$ fail.}
\label{tab:oracle}
\centering
\footnotesize
\setlength{\tabcolsep}{3.2pt}
...
When a DSP specification admits multiple valid realizations, a
reference implementation is one point in $\mathcal{V}_t$.
...
Correctness of a generated DSP implementation is specification-set
membership.
